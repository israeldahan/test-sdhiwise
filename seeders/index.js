...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Carson24' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'V3DwbGEKExCBv3u',
        'isDeleted':false,
        'username':'Carson24',
        'email':'Melany.Marks@yahoo.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Carson24' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Mylene89' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'WrP2NzkwRIsSNUc',
        'isDeleted':false,
        'username':'Mylene89',
        'email':'Sheridan_Gusikowski43@yahoo.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Mylene89' }, userToBeInserted);
    }
